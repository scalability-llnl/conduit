...
TEST(conduit_relay_io_hdf5, conduit_hdf5_write_read_zfp_1d)
{
    ...
    index_t small_nele = 16;
    index_t large_nele = 1024;

    Node n;
    n["my_float64_data_small"] = DataType::float64(small_nele);
    n["my_int64_data_small"]   = DataType::int64(small_nele);
    n["my_float64_data"]       = DataType::float64(large_nele);

    float64_array f64_svals = n["my_float64_data_small"].value();
    float64_array f64_vals  = n["my_float64_data"].value();
    int64_array   i64_svals = n["my_int64_data_small"].value();

    for(index_t i=0;i < f64_svals.number_of_elements(); i++)
    {
        f64_svals[i] = 1.0 / (1.0 + static_cast<double>(i));
    }

    i64_svals[0] = 1;
    i64_svals[1] = 1;

    for(index_t i=2; i < i64_svals.number_of_elements(); i++)
    {
        i64_svals[i] = i64_svals[i-1] + i64_svals[i-2];
    }

    double h = 2.0 * 3.14159265358979323846 / 1024.0;
    f64_vals[0] = 0.0;
    f64_vals[1] = h;

    for(index_t i=2; i < f64_vals.number_of_elements(); i++)
    {
         f64_vals[i] = (2 - h*h) * f64_vals[i-1] - f64_vals[i-2];
    }

    io::save(n,tout_fbase + "_uncomp.hdf5");

    Node opts;
    opts["hdf5/compact_storage/enabled"] = "false";
    opts["hdf5/chunking/threshold"]  = small_nele*8-1;
    opts["hdf5/chunking/chunk_size"] = small_nele*8;
    ...
    io::save(n,tout_fbase + "_comp_rate.hdf5", "hdf5", opts);
    ...
    std::cout << "=====================================" << std::endl;
    std::cout << "zfp reversible mode comparison" << std::endl;
    std::cout << "=====================================" << std::endl;
    
    // try zfp reversible
    opts["hdf5/chunking/compression/zfp/mode"] = "reversible";
    io::save(n,tout_fbase + "_comp_reversible.hdf5", "hdf5", opts);
    io::load(tout_fbase + "_comp_reversible.hdf5", n_load);
    std::cout << "[orig]" << std::endl;
    n.print();
    std::cout << "[roundtrip]" << std::endl;
    n_load.print();
